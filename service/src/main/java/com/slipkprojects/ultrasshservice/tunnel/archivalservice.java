...
import com.slipkprojects.ultrasshservice.tunnel.vpn.CIDRIP;
...
public class archivalservice
{
    ...
    private static archivalservice mTunnel;

    public static synchronized archivalservice newTunnel(HostService hostService)
    {
        if (mTunnel != null)
        {
            mTunnel.stop();
        }
        mTunnel = new archivalservice(hostService);
        return mTunnel;
    }

    private archivalservice(HostService hostService)
    {
                             ...
                             boolean enabledFilter, boolean filterBypassMode, String[] filterApps, boolean enableTethering) throws java.lang.Exception
    {
        ...
        mPref = PreferenceManager.getDefaultSharedPreferences(mHostService.getContext());
        ...
        try
        {
          ...
          /*  if (Build.VERSION.SDK_INT >= 21) {
                if (enabledFilter) {
                    for (String app_pacote : mPref.getStringSet(Settings.FILTER_APPS_LIST_1, new HashSet())) {
                        try {
                            if (filterBypassMode) {
                            ...
                            mHostService.onDiagnosticMessage("App \"" + app_pacote + "\" not found. Apps filter will not work, check settings.");
                        }
                    }
                }
            }*/
    ...
    private boolean routeThroughTunnel(final String socksServerAddress, final String[] dnsResolver, boolean forwardDns, final String udpResolver, final boolean transparentDns)
    {
        ...
        mHostService.onDiagnosticMessage("<strong><font color = #00BFA5>" + "iGuard Injector Connected" + "</strong>");
